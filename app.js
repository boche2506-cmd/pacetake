    ...
    try {
        ...
        if (storeData) {
            ...
            export function initCartDOMState() {
                const localCartData = getCartData(); // 這會從 localStorage 拿資料
                const currentStoreId = document.body.getAttribute('data-store-id');

                console.log("=== [除錯] 開始回填狀態 ===");
                console.log("當前網頁商店 ID:", currentStoreId);
                console.log("LocalStorage 內的所有資料:", localCartData);

                if (!localCartData || localCartData.length === 0) {
                    console.log("LocalStorage 是空的，沒有東西可以回填。");
                    return;
                }

                localCartData.forEach(item => {
                    // 比對邏輯
                    const isMatch = (item.storeId === currentStoreId);
                    console.log(`正在比對: ${item.name} | 商品內存的 storeId: ${item.storeId} | 比對結果: ${isMatch}`);
                    ...
                    if (card) {
                        console.log(`✅ 成功找到卡片: ${item.name}，準備回填數量: ${item.qty}`);
                        ...
                        console.warn(`❌ 找不到對應的 DOM 卡片，可能是 ID 不匹配，商品 ID 為: ${item.id}`);
                    }
                });
                console.log("=== [除錯] 回填狀態結束 ===");
            }
        };
    }
    catch (error) {
