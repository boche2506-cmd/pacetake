...
async function fetchStoresFromFirebase() {
    try {
        ...
        if(storeContainer) storeContainer.innerHTML = '<div class="store-container" style="color:var(--brand-red);">❌ 無法取得雲端店家資料</div>';
    }
}
...
function filterAndRenderStores() {
    ...
    if (filtered.length === 0) {
        storeContainer.innerHTML = '<div class="store-container">🍃 此商圈目前尚無合作店家進駐喔！</div>';
        return;
    }
